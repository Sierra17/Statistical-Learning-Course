...
l_val_glm_store = numeric(dim(df_test)[1])

for (j in 1:dim(df_test)[1]){
  aux1 = df_test.01[j] * log(predictions_glm[j])
  aux2 = (1-df_test.01[j]) * log(1- predictions_glm[j])
...
l_val_lasso_store = numeric(dim(df_test)[1])

for (j in 1:dim(df_test)[1]){
  aux1 = df_test.01[j] * log(predictions_lasso[j])
  aux2 = (1-df_test.01[j]) * log(1- predictions_lasso[j])
...
The most relevant predictors to detect NO spam are: "george", "meeting", "george", "hp"
The most relevant predictors to detect spam are: "$", "000", "money", "!"
Some discarded predictors are: "mail", "receive", "addresses" and "telnet"
...
From these observations, we can observe which predictors are related to NO spam (negative ones)
and the ones related to spam (positive ones) and ordered.
The most relevant predictors to detect NO spam are: "data", "conference", "meeting", "hp"
The most relevant predictors to detect spam are: "$", "!", "remove", "000"
Some discarded predictors are: "make", "all", "3d" and "address"
...
for(j in 1:length(k_vec)){
  ...
  print("missclassification rate for LASSO ussing c = 1/2 and k = : ");print(k_vec[j]);
...
predicted_classes_knn <- ifelse(predictions_knn >= 0.5, 1, 0)
misclassification_rate_knn <- mean(predicted_classes_knn != df_test.01)
print("missclassification rate for LASSO ussing c = 1/2 and k = : ");print(k);
  print("is: "); print(misclassification_rate_knn )

l_val_knn_store = numeric(dim(df_test)[1])

for (j in 1:dim(df_test)[1]){
  aux1 = df_test.01[j] * log(predictions_knn[j])
  aux2 = (1-df_test.01[j]) * log(1- predictions_knn[j])
